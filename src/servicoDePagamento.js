...
export default class ServicoDePagamento {
    ...
    pagar(codigoDeBarra, empresa, valorPagamento) {
        if (!codigoDeBarra || !empresa || valorPagamento == undefined || valorPagamento == null) {
            throw new Error('Informar todos os dados necessários para pagamento.');
        }

        if (typeof codigoDeBarra !== 'string' || typeof empresa !== 'string' || typeof valorPagamento !== 'number') {
            throw new Error('Dados informados estão inválidos.');
        }

        if (valorPagamento <= 0 ) {
            throw new Error('Valor informado deve ser um número maior ou igual a zero.');
        }

        if (this.#pagamentos.some(pagamento => pagamento.codigobarras === codigoDeBarra)) {
            throw new Error('Já existe pagamento realizado para o Código de barras informado.');
        }
